...
<svelte:head>
  <title>Analytics & Relatórios - SIS Lens</title>
  <meta
    name="description"
    content="Dashboard de métricas e insights de negócio com dados em tempo real"
  />
</svelte:head>
      ...
      {#if economia && Object.keys(economia).length > 0}
        <section class="mt-12">
          <SectionHeader
            ...
            subtitle="Onde você está economizando mais"
          />
            ...
            >
              <svelte:fragment slot="cell" let:row let:column>
                ...
                {:else if column === "score_qualidade"}
                  <Badge
                    variant={getQualidadeColor(row.score_qualidade || 0)}
                    size="sm"
                  >
