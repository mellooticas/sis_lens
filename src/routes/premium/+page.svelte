<script lang="ts">
    /**
     * Premium Collection v4 — Sidebar unificada (padrão CanonicalFilterSidebar)
     *
     * Fonte: catalog_lenses.rpc_premium_filter_options + rpc_premium_search.
     * Mantém o tema amber (premium). Reutiliza CanonicalFilterSidebar.svelte.
     */
    ...
    // Estado do filtro (espelha URL)
    $: filtros = {
        brand:        data.brand        ?? null,
        productLine:  data.product_line ?? null,
    ...
    async function fetchFilterOptions() {
        ...
        const res = await LensOracleAPI.getPremiumFilterOptions({
            brand:        filtros.brand        || undefined,
            product_line: filtros.productLine  || undefined,
    ...
    function navegar(next: typeof filtros) {
        ...
        if (next.brand)        p.set('marca',    next.brand);
        if (next.productLine)  p.set('linha',    next.productLine);
        if (next.lensType)     p.set('tipo',     next.lensType);
        if (next.coating)      p.set('coating',  next.coating);
    ...
    function irParaPagina(pg: number) {
        ...
        if (filtros.brand)        params.set('marca',    filtros.brand);
        if (filtros.productLine)  params.set('linha',    filtros.productLine);
        if (filtros.lensType)     params.set('tipo',     filtros.lensType);
        if (filtros.coating)      params.set('coating',  filtros.coating);
        ...
        <Container>
            <div class="py-6 flex items-start justify-between gap-4 flex-wrap">
                <div>
                    ...
                    <h1 class="text-2xl font-black text-foreground mt-1">Premium Collection</h1>
                    <p class="text-sm text-muted-foreground mt-1">
                        {#if loading}Carregando…{:else}{total.toLocaleString('pt-BR')} conceitos · marcas consagradas{/if}
                    </p>
                </div>
                        ...
                        {#each items as c (c.id)}
                            <a href="/premium/{c.id}"
                                ...
                                <div class="flex items-start justify-between gap-2">
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-center gap-2 mb-1 flex-wrap">
                                            ...
                                            {#if c.product_line}
                                                <span class="text-[10px] font-bold text-muted-foreground truncate">{c.product_line}</span>
                                            {/if}
                                        </div>
                                ...
                                <div class="space-y-1 flex-1">
                                    ...
                                    {#if c.coating_name}
                                        <p class="text-xs text-muted-foreground"><span class="font-semibold">Coating:</span> {c.coating_name}</p>
                                    {/if}
                                    {#if c.photochromic_type}
                                        <p class="text-xs text-amber-700 dark:text-amber-300"><span class="font-semibold">Foto:</span> {c.photochromic_type}</p>
                                    {/if}
                                </div>

                                <!-- Chips coloridos de tratamentos (união das lentes reais mapeadas) -->
                                ...
                                <div class="flex items-center justify-between pt-3 border-t border-border">
                                    ...
                                    {#if c.price_avg}
                                        <span class="text-xs font-bold text-amber-700 dark:text-amber-300">{fmt(c.price_avg)}</span>
                                    {/if}
                                </div>
                            </a>
                        {/each}
