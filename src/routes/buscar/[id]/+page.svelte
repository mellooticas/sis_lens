...
<script lang="ts">
  ...
  async function carregarDetalhes() {
    try {
      ...
      if (resultado.success && resultado.data && resultado.data.dados.length > 0) {
        lente = resultado.data.dados[0];
      } else {
...
<Container maxWidth="xl" padding="md">
  ...
  {:else if error || !lente}
    <div class="bg-red-50 border-2 border-red-200 rounded-2xl p-8 text-center">
      ...
      <Button variant="primary" on:click={() => goto('/buscar')}>
        Voltar para Busca
      </Button>
    </div>
      ...
      <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
        <div class="flex-1">
          ...
          {#if lente.linha_produto}
            <p class="text-lg text-slate-600 mb-4">Linha: {lente.linha_produto}</p>
          {/if}

          {#if lente.descricao_curta}
            <p class="text-slate-700 text-lg">{lente.descricao_curta}</p>
          {/if}
        </div>

        <div class="text-right">
          <div class="bg-gradient-to-br from-indigo-600 to-purple-600 text-white rounded-2xl p-6 min-w-[200px]">
            <div class="text-sm opacity-90 mb-1">Preço Tabela</div>
            <div class="text-3xl font-bold">{formatarPreco(lente.preco_tabela)}</div>
            {#if lente.preco_fabricante}
              <div class="text-xs opacity-75 mt-2">Fabricante: {formatarPreco(lente.preco_fabricante)}</div>
            {/if}
          </div>
        </div>
      </div>
