<script lang="ts">
  ...
  function filtrarDecisoes(decisoes: any[]) {
    return decisoes.filter(decisao => {
      ...
      const matchCriterio = !filtroCriterio || decisao.criterio === filtroCriterio;
      ...
      return matchBusca && matchLab && matchCriterio && matchDataInicio && matchDataFim;
    });
  }
...
<svelte:head>
  ...
  <meta name="description" content="Histórico completo de todas as decisões de compra realizadas" />
</svelte:head>
    ...
    <Container maxWidth="xl" padding="md">
      ...
      <section class="mt-12">
        <SectionHeader
          ...
          subtitle={decisoesFiltradas.length ? `${decisoesFiltradas.length} decisões encontradas` : ''}
        />
        ...
        {:else if decisoesPaginadas && decisoesPaginadas.length > 0}
          ...
          >
            <svelte:fragment slot="cell" let:row let:header>
              ...
              {:else if header.key === 'economia'}
                <span class="font-medium" class:text-success={row.economia > 0} class:text-neutral-500={row.economia <= 0}>
              ...
              {:else if header.key === 'actions'}
                <div class="flex gap-2 justify-center">
                  ...
                  <Button
                    variant="secondary"
                    size="sm"
                    on:click={() => goto(`/ranking/${row.lente_id}?criterio=${row.criterio}`)}
                  >
                    🔄 Repetir
                  </Button>
                </div>
          ...
          {#if totalPaginas > 1}
            <div class="flex justify-center items-center gap-2 mt-6">
              ...
              <span class="px-4 py-2 text-sm text-neutral-600 dark:text-neutral-400">
