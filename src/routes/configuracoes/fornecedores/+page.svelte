<script lang="ts">
  ...
  $: configuracoesFiltradas = data.configuracoes.filter(config => {
    const buscaMatch = !filtros.busca || 
      config.fornecedor_nome.toLowerCase().includes(filtros.busca.toLowerCase());
    
    const statusMatch = !filtros.status || config.status === filtros.status;
    
    const regiaoMatch = !filtros.regiao || config.regiao_atendimento === filtros.regiao;
  ...
  async function salvarConfiguracao() {
    ...
    try {
      ...
      const index = data.configuracoes.findIndex(c => c.id === editingConfig.id);
      if (index !== -1) {
...
<svelte:head>
  ...
  <meta name="description" content="Gerencie configurações específicas dos fornecedores e laboratórios" />
</svelte:head>
        ...
        <div class="bg-white dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <Select
              ...
              label="Status"
              options={[{value: '', label: 'Todos'}, ...data.opcoes.status_opcoes]}
            />
            
            <Select
              bind:value={filtros.regiao}
              label="Região"
              options={[{value: '', label: 'Todas'}, ...data.opcoes.regioes.map(r => ({value: r, label: r}))]}
            />
    ...
    <div slot="body" class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Select
          ...
          options={data.opcoes.tipos_pagamento.map(t => ({value: t, label: t}))}
          required
        />
      </div>
