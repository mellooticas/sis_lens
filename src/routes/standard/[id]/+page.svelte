...
<svelte:head>
    <title>{conceito.canonical_name} — Standard | Clearix Lens</title>
</svelte:head>
    ...
    <div class="bg-card border-b border-border sticky top-0 z-30">
        <Container>
            <div class="flex items-center justify-between py-4">
                <a
                    href="/lentes"
                    class="flex items-center gap-2 text-muted-foreground hover:text-cyan-600 transition-colors text-sm font-medium"
                >
                    <ChevronLeft class="h-4 w-4" /> Voltar
                </a>
                <span
                    class="px-3 py-1 bg-cyan-100 dark:bg-cyan-900/40 text-cyan-800 dark:text-cyan-300 text-xs font-bold rounded-full"
                >
                    Standard
                </span>
            </div>
        ...
        <div class="py-12">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-6">
                    <div>
                        <p class="text-sm font-bold uppercase tracking-widest text-cyan-600 dark:text-cyan-400 mb-2">
                            Conceito Canônico Standard
                        </p>
                        <h1 class="text-4xl font-black bg-gradient-to-r from-cyan-600 to-blue-600 bg-clip-text text-transparent">
                    ...
                    <div class="bg-card border border-border rounded-xl p-6">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                            ...
                            {#if conceito.treatment_codes && conceito.treatment_codes.length > 0}
                                <div class="col-span-2 sm:col-span-3">
                                    <p class="text-xs text-muted-foreground mb-1">Tratamentos</p>
                                    <div class="flex flex-wrap gap-2">
                                        {#each conceito.treatment_codes as code}
                                            <span class="px-2 py-0.5 text-xs font-medium bg-cyan-100 dark:bg-cyan-900/40 text-cyan-800 dark:text-cyan-300 rounded">
                                                {code}
                                            </span>
                                        {/each}
                                    </div>
                ...
                <div class="lg:col-span-1">
                    <div class="bg-card border border-cyan-100 dark:border-cyan-900/30 rounded-xl p-6 sticky top-20">
