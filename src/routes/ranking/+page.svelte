<script lang="ts">
  ...
  function getBadgeVariant(badge: string): 'primary' | 'success' | 'warning' | 'orange' | 'gold' {
  ...
  function toggleComparacao(opcao: any) {
    const index = selectedForComparison.findIndex(item => item.id === opcao.id);
    if (index === -1) {
      if (selectedForComparison.length < 3) {
        selectedForComparison = [...selectedForComparison, opcao];
      }
    } else {
      selectedForComparison = selectedForComparison.filter(item => item.id !== opcao.id);
    }
  }
  ...
  function aplicarFiltros() {
    ...
    if (filtrosAtivos.cidade) newUrl.searchParams.set('cidade', filtrosAtivos.cidade);
    else newUrl.searchParams.delete('cidade');
    
    if (filtrosAtivos.prazo_maximo) newUrl.searchParams.set('prazo_maximo', filtrosAtivos.prazo_maximo);
    else newUrl.searchParams.delete('prazo_maximo');
    
    if (filtrosAtivos.preco_maximo) newUrl.searchParams.set('preco_maximo', filtrosAtivos.preco_maximo);
...
<svelte:head>
  ...
  <meta name="description" content="Ranking inteligente de fornecedores baseado em critérios customizáveis" />
</svelte:head>
...
<div class="min-h-screen bg-neutral-50 dark:bg-neutral-900 transition-colors">
  ...
  <main>
    <Container maxWidth="2xl" padding="lg">
      ...
      {#if lente_info}
        <section class="mt-8">
          <div class="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 rounded-2xl border border-blue-200 dark:border-blue-800 p-8 shadow-lg">
            <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
              <div class="flex-1">
                ...
                <div class="text-sm text-neutral-600 dark:text-neutral-400">
                  <strong>SKU:</strong> {lente_info.sku_canonico} · 
                  <strong>Marca:</strong> {lente_info.marca_nome || 'N/A'}
                </div>
              </div>
      ...
      <section class="mt-12">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          {#each criterios as crit}
            <button
              class="text-left p-6 rounded-xl border-2 transition-all duration-200 {
                criterioAtual === crit.id
              ...
              <div class="relative">
                ...
                <div class="absolute -top-4 -right-4 z-10">
                  <div class="w-12 h-12 rounded-full bg-gradient-to-br {
                    ...
                    <span class="text-white font-bold text-lg">{opcao.posicao}º</span>
                  </div>
                </div>
                
                <!-- Card -->
                <div class="bg-white dark:bg-neutral-800 rounded-2xl border-2 {
                  index === 0 ? 'border-yellow-500 shadow-2xl' :
                  index === 1 ? 'border-gray-400 shadow-xl' :
                  'border-orange-400 shadow-lg'
                } p-6 transition-all duration-200 hover:scale-105">
                  
                  <!-- Cabeçalho -->
                  <div class="mb-4">
                    <h4 class="text-xl font-bold text-neutral-900 dark:text-neutral-100 mb-2">
                      {opcao.laboratorio_nome}
                    </h4>
                    <div class="flex items-center gap-2 text-sm text-neutral-600 dark:text-neutral-400">
                      <span>📍 {opcao.laboratorio_cidade || 'N/A'}, {opcao.laboratorio_uf || 'N/A'}</span>
                  ...
                  <div class="grid grid-cols-3 gap-2 mb-4">
                    <div class="text-center p-2 bg-neutral-50 dark:bg-neutral-700 rounded-lg">
                      <div class="text-xs text-neutral-600 dark:text-neutral-400">Preço</div>
                      <div class="font-semibold text-neutral-900 dark:text-neutral-100">
                        {opcao.score_preco}
                      </div>
                    </div>
                    <div class="text-center p-2 bg-neutral-50 dark:bg-neutral-700 rounded-lg">
                      <div class="text-xs text-neutral-600 dark:text-neutral-400">Prazo</div>
                      <div class="font-semibold text-neutral-900 dark:text-neutral-100">
                        {opcao.score_prazo}
                      </div>
                    </div>
                    <div class="text-center p-2 bg-neutral-50 dark:bg-neutral-700 rounded-lg">
                      <div class="text-xs text-neutral-600 dark:text-neutral-400">Qualidade</div>
                      <div class="font-semibold text-neutral-900 dark:text-neutral-100">
                  ...
                  <!-- Score Total e Prazo -->
                  <div class="flex items-center justify-between mb-4 p-3 bg-blue-50 dark:bg-blue-900/30 rounded-lg">
                    <div>
                      <div class="text-xs text-neutral-600 dark:text-neutral-400">Score Total</div>
      ...
      {#if selectedForComparison.length >= 2}
        <section class="mt-12">
          <div class="bg-blue-50 dark:bg-blue-900/20 border-2 border-blue-200 dark:border-blue-800 rounded-xl p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-{selectedForComparison.length} gap-4">
              {#each selectedForComparison as item}
                <div class="bg-white dark:bg-neutral-800 rounded-lg p-4 border border-neutral-200 dark:border-neutral-700">
                  <div class="font-semibold text-neutral-900 dark:text-neutral-100 mb-2">
            ...
            <div class="overflow-x-auto">
              <table class="w-full">
                <thead class="bg-neutral-50 dark:bg-neutral-700">
                  <tr>
                    <th class="px-6 py-3 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">#</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">Laboratório</th>
                    <th class="px-6 py-3 text-right text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">Preço</th>
                    <th class="px-6 py-3 text-center text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">Prazo</th>
                    <th class="px-6 py-3 text-center text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">Score</th>
                    <th class="px-6 py-3 text-center text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">Ações</th>
                  </tr>
                </thead>
                  ...
                  {#each outros as opcao}
                    <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-700 transition-colors">
                      <td class="px-6 py-4 whitespace-nowrap">
                        <Badge variant={getPosicaoColor(opcao.posicao)} size="sm">
                          {opcao.posicao}º
                        </Badge>
                      </td>
                      <td class="px-6 py-4">
                        ...
                        <div class="text-sm text-neutral-500">
                          {opcao.laboratorio_cidade || 'N/A'}, {opcao.laboratorio_uf || 'N/A'}
                        </div>
                      </td>
    ...
    <div class="space-y-6">
      <div class="bg-blue-50 dark:bg-blue-900/30 rounded-xl p-6">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            <span class="text-sm text-neutral-600 dark:text-neutral-400">Preço Final</span>
            <p class="text-2xl font-bold text-success">
              {formatCurrency(selectedOption.preco_final)}
            </p>
          </div>
          <div>
            <span class="text-sm text-neutral-600 dark:text-neutral-400">Prazo</span>
        ...
        <div class="mt-4 pt-4 border-t border-neutral-200 dark:border-neutral-700">
          <div class="flex items-center justify-between">
            <span class="text-sm text-neutral-600 dark:text-neutral-400">Score Total</span>
            <Badge variant={getScoreColor(selectedOption.score_total)} size="md">
      ...
      <div>
        <label for="observacoes" class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">
      ...
      <div class="bg-neutral-50 dark:bg-neutral-700 rounded-lg p-4">
        <p class="text-sm text-neutral-700 dark:text-neutral-300">
          <strong>💡 Justificativa:</strong> {selectedOption.justificativa}
        </p>
      </div>
    </div>
...
{#if showFiltersModal}
  ...
  >
    <div class="space-y-4">
      <div>
        <label for="filtro-uf" class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">
          Estado (UF)
        </label>
        <select
          id="filtro-uf"
      ...
      <div>
        <label for="filtro-cidade" class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">
          Cidade
        </label>
        <input
          id="filtro-cidade"
      ...
      <div>
        <label for="filtro-prazo" class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">
          Prazo Máximo (dias)
        </label>
        <input
          id="filtro-prazo"
      ...
      <div>
        <label for="filtro-preco" class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">
          Preço Máximo (R$)
        </label>
        <input
          id="filtro-preco"
