...
{#if open}
  ...
  <!-- Drawer -->
  <aside class="mobile-menu-drawer" transition:fly={{ x: -300, duration: 300 }}>
    <!-- Header -->
    <div class="drawer-header">
      <Logo size="md" />

      <button class="close-btn" on:click={handleClose} aria-label="Fechar menu">
        <svg
          width="24"
          height="24"
          fill="none"
    ...
    <div class="drawer-footer">
      ...
      <!-- User Info (SSO Gateway via currentUser) -->
      <div class="user-btn">
        ...
        <div class="user-info">
          <div class="user-name">
            {$currentUser ? $currentUser.firstName : "Usuário"}
          </div>
          <div class="user-email">
            {$currentUser ? $currentUser.tenantName : "Conectando..."}
          </div>
        </div>
      </div>
    </div>
